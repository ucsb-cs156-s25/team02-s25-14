...
describe("RecommendationRequestIndexPage tests", () => {
  ...
  test("Renders with Create Button for admin user", async () => {
    ...
    axiosMock.onGet("/api/recommendation-requests/all").reply(200, []);
  ...
  test("renders three recommendation requests correctly for regular user", async () => {
    ...
    const queryClient = new QueryClient();
    axiosMock
      .onGet("/api/recommendation-requests/all")
  ...
  test("renders empty table when backend unavailable, user only", async () => {
    ...
    setupUserOnly();
    const queryClient = new QueryClient();
    axiosMock.onGet("/api/recommendation-requests/all").timeout();
    ...
    expect(errorMessage).toMatch(
      "Error communicating with backend via GET on /api/recommendation-requests/all",
    );
    restoreConsole();
  ...
  test("what happens when you click delete, admin", async () => {
    ...
    axiosMock
      ...
      .reply(200, { message: "RecommendationRequest with id 2 deleted" });
